package com.example.chaoren.myapplication.view;
        ...
        if (isSwipeEnable) {
            ...
            switch (ev.getAction()) {
                case MotionEvent.ACTION_DOWN:
                    ...
                    mLastP.set(ev.getRawX(), ev.getRawY());//屏幕的位置 getRawX相对于屏幕的坐标
...
//                    if (Math.abs(gap) > Math.abs(gapy) || Math.abs(getScrollX()) > Math.abs(getScrollY())) {// 原来是10、、、2016 09 29 修改此处，使屏蔽父布局滑动更加灵敏，
//                    getParent().requestDisallowInterceptTouchEvent(true);
//                    }
                   if (Math.abs(gap) > Math.abs(gapy) && Math.abs(getScrollX()) > Math.abs(getScrollY())) {
                        getParent().requestDisallowInterceptTouchEvent(true);
                    }else{
...
/*        mScroller.startScroll(getScrollX(), 0, -getScrollX(), 0);
    ...
    //每次ViewDetach的时候，判断一下 。，如果是自己，关闭侧滑菜单，且ViewCache设置为null，
